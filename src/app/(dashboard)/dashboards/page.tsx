...
function useSites() {
  return useQuery<Array<{ website_id: string; name: string; domain: string }>>({
    queryKey: ["sites"],
    queryFn: async () => {
    ...
    <>
      ...
      <div className="flex-1 p-4 space-y-4">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={siteId} onValueChange={setSiteId}>
                    ...
                    <SelectContent>
                      {sites?.map((s) => (
                        <SelectItem key={s.website_id} value={s.website_id}>
                          {s.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {dashboards.map((d) => {
              const site = sites?.find((s) => s.website_id === d.websiteId);
              return (
                <Card
                  key={d.id}
