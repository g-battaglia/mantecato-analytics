...
app = typer.Typer(
    name="mantecato", help="Mantecato Analytics CLI", no_args_is_help=True
)
console = Console()
...
def sites(
    format: str = typer.Option(
        "table", "-f", "--format", help="Output: json, table, csv"
    ),
):
...
def report_cmd(
    ...
    human: bool = typer.Option(
        False, "-H", "--human", help="Human-friendly output with tables and bars"
    ),
):
    """Full analytics report in one shot."""
    from mantecato_cli.report import run_report
...
def page_detail(
    ...
    async def _run():
        ...
        referrers, next_pages, distribution, ts = await asyncio.gather(
            get_page_referrers(
                site_id, url, date_range.start_date, date_range.end_date, limit
            ),
            get_next_pages(
                site_id, url, date_range.start_date, date_range.end_date, limit
            ),
            get_time_on_page_distribution(
                site_id, url, date_range.start_date, date_range.end_date
            ),
            get_page_time_series(
                site_id, url, date_range.start_date, date_range.end_date, gran, filters
            ),
        )
        data = {
            "referrers": referrers,
...
def referrer_pages(
    ...
    async def _run():
        from mantecato_core.queries.sources import get_referrer_pages as _get
...
def devices(
    ...
    dimension: str = typer.Option(
        "device", "--dimension", help="browser, os, device, screen, language"
    ),
    period: str = typer.Option("30d", "-p", "--period"),
    start: str = typer.Option(None, "--start"),
    end: str = typer.Option(None, "--end"),
...
def funnel(
    ...
    steps: str = typer.Option(
        ..., "--steps", help="Comma-separated URLs or event names"
    ),
    ...
    async def _run():
        ...
        site_id = await resolve_site_id(site)
        date_range = parse_date_args(period, start, end)
        step_list = [{"type": "url", "value": s.strip()} for s in steps.split(",")]
...
def revenue(
    ...
    view: str = typer.Option(
        "summary", "--view", help="summary, timeseries, by-event, by-country"
    ),
    period: str = typer.Option("30d", "-p", "--period"),
    start: str = typer.Option(None, "--start"),
    end: str = typer.Option(None, "--end"),
...
def engagement(
    ...
    view: str = typer.Option(
        "percentiles",
        "--view",
        help="distribution, percentiles, by-page, bounce-by-page, bounce-by-source",
    ),
    period: str = typer.Option("30d", "-p", "--period"),
    start: str = typer.Option(None, "--start"),
    end: str = typer.Option(None, "--end"),
...
def filter_values(
    ...
    async def _run():
        ...
        console.print(
            format_output(formatted, format, title=f"Filter Values: {column}")
        )
...
def annotation_create(
    ...
    async def _run():
        ...
        user_id = await resolve_user_id(api_key)
        site_id = await resolve_site_id(site)
        data = await create_annotation(
            user_id, site_id, title, description, date, color
        )
...
config_app = typer.Typer(
    name="config", help="Manage local configuration", no_args_is_help=True
)
app.add_typer(config_app, name="config")


@config_app.command("get")
def config_get(key: str = typer.Argument(..., help="Config key (e.g. database.url)")):
    """Get a config value."""
    from mantecato_cli.config import get_value
...
def config_list():
    ...
    from mantecato_cli.config import list_values

    config = list_values()
    if not config:
        console.print("[dim]No config values set[/]")
        return
...
@app.command()
def tui(
    site: str = typer.Option(None, "-s", "--site", help="Site name, domain, or UUID"),
    period: str = typer.Option(None, "-p", "--period", help="Date range preset"),
):
    """Launch the terminal dashboard UI."""
    from mantecato_cli.tui import run_tui

    run_tui(site=site, period=period)
