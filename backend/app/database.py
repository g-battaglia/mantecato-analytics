...
# Match both {{param::type}} (from regular strings) and {param::type}
# (the result of {{param::type}} inside an f-string where {{ -> {).
...
def _substitute_params(sql: str, params: dict[str, Any]) -> tuple[str, list[Any]]:
    """
    Replace {{param::type}} or {param::type} placeholders with $1, $2, ...
    positional parameters for asyncpg.

    Supports both double-brace {{param}} (from regular strings) and
    single-brace {param} (after f-string expansion of {{ -> {).
    """
    args: list[Any] = []

    def _replacer(m: re.Match) -> str:
        # group(1)/group(2) for {{param::type}}, group(3)/group(4) for {param::type}
